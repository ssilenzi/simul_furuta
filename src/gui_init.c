...
void gui_init()
{
	...
	rect(screen, dist + wrett1 + dist , dist + hrett2 + hrett3 + dist , dist + wrett1 + dist + wrett5,  dist + hrett2 + hrett3 + dist + hrett5, colrett);
		...
		textout_ex(screen, font, exit, dist + wrett1 + dist + dist ,dist + hrett2 + hrett3 + dist + dist, colscr, colbck);
		sprintf(reset, "R per resettare");
		textout_ex(screen, font, reset, dist + wrett1 + dist + dist , dist + hrett2 + hrett3 + dist + dist + dist, colscr, colbck);
}
